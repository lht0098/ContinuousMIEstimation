...
for i = 1:size(x,3)
    ...
    % 202205003 LHT: output the stabiility weight for first four data frac
    disp(stability_weight)
    ...
    % 202205003 LHT: output the stability weight for total comparisons
    disp(n_stableFracs/nFracPairs)
     % 202205003 LHT: save into cell array
...
if any(weighted_k >= 1) 
    ...
    if length(quality_Ks) >= 1 % Should this be only stable ks? 
        ...
        if ~f_stableKs_only
            ...
            if length(quality_Ks) > 0; k_stability_weights(quality_Ks) = 1; end
        end
    end
...
if all(weighted_k < 2)
    ...
    k_stab_neighbor_comp = zeros(1,length(k_stab_neighbor)); % initate an array to hold thresh eval
    k_stab_neighbor_op = zeros(1,length(k_stab_neighbor)); % initiate an array to hold stability eval 
    for i=1:length(k_stab_neighbor)
        ...
        if k_stab_neighbor_comp(i) == 1 % check that both neighbors are within thresh
           k_stab_neighbor_op(i) = mean(k_stab_neighbor(1,i) - k_stab_neighbor(2,i)); % evaluate the average std
        else 
    ...
    k_stab_neighbor_i(lowest_err) % get the k val that has the highest stability with neighbors
